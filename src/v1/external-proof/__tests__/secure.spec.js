...
it('issue for single subject', async () => {
  const verifiableCredentials = await v1.proof.credential.secure({
    credential: validCredentialFragment,
    privateKey: privateKey,
  });
  expect(verifiableCredentials.length).toBe(1);
  const result = await v1.proof.credential.verify({
...
it('issue for multiple subjects', async () => {
  ...
  const result1 = await v1.proof.credential.verify({
    verifiableCredential: verifiableCredentials[0],
    publicKey: publicKey,
  });
  expect(result1.verified).toBe(true);

  const result2 = await v1.proof.credential.verify({
    verifiableCredential: verifiableCredentials[0],
    publicKey: publicKey,
  });
